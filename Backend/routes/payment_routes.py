...
@payment_bp.route("/payment/invoice/<int:document_id>", methods=["GET", "OPTIONS"])
def download_invoice(document_id):
    """
    Download invoice PDF using document_id.
    ...
    Security: The document_id is sent in the email link. While not as secure
    as retrieval_key, it's still reasonably safe since users need the email.

    Args:
        document_id: Document ID from invoice creation
    ...
    # Validate document_id format (basic sanity check)
    ...
    try:
        pdf_content = billing_service.download_invoice_pdf(document_id)
