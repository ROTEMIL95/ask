...
def ask():
    ...
    "1. NEVER use variables for URLs in fetch() calls\n"
    "2. ALWAYS write the full URL directly as a string in fetch()\n"
    "3. DO NOT use: const apiUrl = 'https://...'; fetch(apiUrl)\n"
    "4. DO NOT use template literals for URLs: const url = `https://${baseUrl}/endpoint`\n"
    "5. ALWAYS use: fetch('https://api.example.com/endpoint', {...})\n"
    "6. Query parameters can be part of the URL string: fetch('https://api.example.com/endpoint?status=available')\n"
    "7. The generated code must be copy-paste executable without defining URL variables\n"
    "\n\n"
    "In JavaScript, you MAY define variables for authentication, query params, or body data, "
    "but NOT for the URL itself. "
    "Always use fetch with an explicit configuration object, never omitting the headers block, "
